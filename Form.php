...
class Form {
  ...
  public function fill_form() {
    if(func_num_args()==2)
     ...
     $formObject = ((object)($this->instances()));
    foreach($formObject as $field=>$obj) { 
      $obj->setValue($this->formValues[$field]);
    }
    $this->validate();
  }
  ...
  /**
   * Return instances that are instance of Mpm\Forms\Field 
   */
  ...
  private function validate(){
    $this->is_valid = true;
    $this->error_list = [];
    foreach($this->instances() as $key=>$field) {
      $field->validate();
      $this->error_list[$key] = $field->getErrors();
      if(!$field->is_valid) $this->is_valid=false;
    }
  }
  
  /**
   * Validates form as a whole . 
   * First , It creates an array that consists an array with it's name , value  and validations . All Data is passed  to 
   * Validator class that returns an associative array with `is_valid` and `error_list` keys . 
   * 
   */
  public function validate_whole(){
  ...
  /**
   * An alias of render_form method
   */
